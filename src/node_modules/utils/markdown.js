import marked from 'marked';
import slugify from 'utils/slugify';

export function parseMarkdown(markdown) {
	const match = /---\r?\n([\s\S]+?)\r?\n---/.exec(markdown);
	const frontMatter = match[1];
	const content = markdown.slice(match[0].length);

    let metadata = {};
    
	frontMatter.split('\n').forEach(keyValuePair => {
        const colonIndex = keyValuePair.indexOf(':');
        const key = keyValuePair.slice(0, colonIndex).trim();
        const value = keyValuePair.slice(colonIndex + 1).trim();
		metadata[key] = value;
	});

	return { metadata, content };
};

export function readMarkdown(markdown, basePath) {
	let { metadata, content } = parseMarkdown(markdown);

	const toc = generateToc(markdown, basePath);
	const renderer = new marked.Renderer();

	renderer.heading = (text, level, raw) => {
		const anchor = `${renderer.options.headerPrefix}${slugify(raw)}`;

		return `<h${level}>
			<a id="${anchor}" href="#${anchor}"></a>
			${text}
		</h${level}>`;
	};

	renderer.link = (href, title, text) => {
		if (href.startsWith('./')) {
			href = `${basePath}/${href}`;
		} else if (href.startsWith('../')) {
			const count = href.match(/..\//g).length;
			const params = basePath.split('/');
			const newBasePath = params.splice(0, params.length - count).join('/');
			const newHref = href.replace(/..\//g, '');
			href = `${newBasePath}/${newHref}`;
		} else if (href.startsWith('#')) {
			const newBasePath = (basePath.endsWith('/')) ? basePath.slice(0, -1) : basePath;
			href = `${newBasePath}${href}`;
		} else if (!isAbsoluteUrl(href) && !href.startsWith('mailto:') && !href.startsWith('/')) {
			href = `${basePath}/${href}`;
		}

		let attributes = `href='${href}'`;

		if (isAbsoluteUrl(href))
			attributes += ` rel='noopener' target='_blank'`;

		if (!!title)
			attributes += ` title='${title}'`;

		return `<a ${attributes}>${text}</a>`;
	};

	content = marked(content, { renderer: renderer });

	return { metadata, toc, content };
};

export function generateToc(markdown, basePath) {
	const renderer = new marked.Renderer();

	let toc = [];

	renderer.heading = (text, level, raw) => {
		const anchor = `${renderer.options.headerPrefix}${slugify(raw)}`;
		
		toc.push({
			anchor: anchor,
			level: level,
			text: text
		});

		return `<h${level}>
			<a id="${anchor}" href="${basePath}#${anchor}"></a>
			${text}
		</h${level}>`;
	};

	marked.setOptions({
		renderer: renderer,
		gfm: true,
		tables: true,
		breaks: false,
		pedantic: false,
		smartLists: true,
		smartypants: false
	});

	const build = (collection, key, level, context) => {
		if (key >= collection.length || collection[key].level <= level)
			return key;
		
		const node = collection[key];

		context.push(`<li><a href="${basePath}#${node.anchor}">${node.text}</a>`);
		key++;

		let childContext = [];

		key = build(collection, key, node.level, childContext);

		if (childContext.length > 0) {
			context.push(`<ul>`);
			childContext.forEach(idm => {
				context.push(idm);
			});
			context.push(`</ul>`);
		}

		context.push(`</li>`);
		
		key = build(collection, key, level, context);

		return key;
	};

	marked(markdown);

	if (!toc || toc.length <= 0)
		return;

	let ctx = [];
	ctx.push(`<ul>`);
	build(toc, 0, 0, ctx);
	ctx.push(`</ul>`);

	return ctx.join('');
};

function isAbsoluteUrl(url) {
    if (url.indexOf('//') === 0) return true;
    if (url.indexOf('://') === -1) return false;
    if (url.indexOf('.') === -1) return false;
    if (url.indexOf('/') === -1) return false;
    if (url.indexOf(':') > url.indexOf('/')) return false;
    if (url.indexOf('://') < url.indexOf('.')) return true;
    return false;
}