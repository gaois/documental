...
| Items           | [`item`](#main-manuscript-item)     | none or one or many | Metadata describing the discrete items which compose the manuscript part. |
...
| ItemID          | integer             | one                 | The identifier of the linked manuscript `item`. |
...
A `part` entity represents a distinct section within a manuscript volume and may have originally been physically separate from the other contents of the volume. It is consonant with the [`msPart`](https://www.tei-c.org/release/doc/tei-p5-doc/en/html/MS.html#mspt) element as specified in the Text Encoding Initiative (TEI) P5 Guidelines. In the Schools' Collection, a `part` contains material collected by a single school. In the case of schools where multiple teachers and/or class groups took part in *Scéim na Scol* folklore collection scheme there may be multiple `part` entities associated with a single school. In the previous Schools' Collection XML-based API manuscript parts were referred to as **chapters**.
...
Due to the scope of the Schools' Collection digitization project it has not yet been feasible to extract a normalised set of personal metadata. Instead, the initial phases of the project sought to capture information about the persons referred to within the collection 'as is'. Thus, potentially, the same individual may be referred to by more than one `person` entity. 
...
| HandbookTopic   | [`handbookTopic`](#handbooktopic)     | none or one         | Denotes the associated subject heading ([topic](https://www.duchas.ie/en/tpc/cbeg)) in Seán Ó Súilleabháin's *A Handbook of Irish Folklore*. |
...
| Physical        | string              | none or one         | The physical character of the image. |
...
| Property name   | Type                | Cardinality         | Description               |
| :-------------- | :------------------ | :------------------ | :------------------------ |
| Copyright       | string              | none or one         | The photograph's copyright holder(s). |
| Condition       | string              | none or one         | The photograph's physical condition. |
| Topic           | string              | none or one         | The associated subject heading ([topic](https://www.duchas.ie/en/tpc/cbeg)) in Seán Ó Súilleabháin's *A Handbook of Irish Folklore*. |
| Date            | string              | none or one         | The date of photograph capture. |
| Photographer    | string              | none or one         | The photographer's name. |
...
| Property name   | Type                | Cardinality         | Description               |
| :-------------- | :------------------ | :------------------ | :------------------------ |
| DateCaptured    | string              | one                 | The date of image digitization. |
...
| Storage         | string              | one                 | The image storage details. |
...
| Property name   | Type                | Cardinality         | Description               |
| :-------------- | :------------------ | :------------------ | :------------------------ |
| FirstNames      | string              | none or one         | A person's first name(s) and, optionally, middle name(s) or nickname(s). |
| Surname         | string              | none or one         | A person's surname.       |
| FullName        | string              | one                 | A person's full name, including first name(s) and surname. |
...
| IsoStartDate    | ISO 8601 datetime   | none or one         | Aggregates the values of the `PeriodStartYear`, `PeriodStartMonth` and `PeriodStartDay` properties below to represent the start date in a time interval. |
| IsoEndDate      | ISO 8601 datetime   | none or one         | Aggregates the values of the `PeriodEndYear`, `PeriodEndMonth` and `PeriodEndDay` properties below to represent the end date in a time interval. |
...
| ItemID          | integer             | one                 | Identifies the manuscript `item` that is transcribed. |
...
| Name            | string              | none or one         | The transcriber's name. If the transcriber subsequently deleted their account, this value is null. |
